...
9. **⛔ 假表資料來源與📱／🏃安排規則(2026-08-13 最新確認)**:
   - **休假表唯一正式來源**=`http://61.216.37.15:8080/`，本站透過 `/api/ext/leave`
     鏡射。休假／必休／每日文字、班別與可休天數都必須完整依照該來源；
     禁止前端寫死、使用 Firebase 舊值或預設天數補齊，來源資料不完整時必須明確顯示同步異常。
   - **📱值公務機由 OMNIPLAY 依鏡射後的出勤資料安排，不由 8080 排定**。
     中魁不參與；早班配對為宋佳臻／熊茗雅，晚班配對為鄭晴心／郭澄希；
     只有同班兩人都上班時才安排，並盡量平均分配。若其中一人未上班，不把📱自動排給另一人。
   - 📱安排必須排除 `🎰、▲、特、病、喪、公出、補(天)`及任何非空白內容。
     📱允許在 OMNIPLAY 休假表手動調整；手動調整是正式功能，不得因重新同步 8080 休假資料而消失。
   - **🏃彈性早退由 OMNIPLAY 配合📱安排**：僅同班兩人都上班時，非📱者才可🏃；
     📱與🏃不得是同一人。病假 2 小時但仍上班者視為有上班；星期三早班依既定會議限制處理。
     🏃可配合📱手動調整。
...
13. **後端 `/api/ext/leave` 鏡射層的真相(2026-08-18)**:正式站的後端是**獨立 Python 服務,不在本 repo**;
   `dev-server/` 只是本機模擬,**merge 進 main 只會自動部署前端,永遠不會改到正式後端**。
   - 正式後端已於 2026-08-18 補上星號轉換:上游頂層 `stars`(結構 `{day, employee_id}`,與 duties 同構)
     → 併入對應人員格子的 `specials` 加 `"event"`;值機 `duties` → `specials:["phone"]`(既有)。
     已用 2026-07(唯一有客服四人星號的月份)逐格驗證與上游一致。
   - 輸出合約:`{month, maxDays, people}`,`people` 同一人有**短名+全名雙 key**,
     每格 `{type, label, specials?}`。
   - ⚠️ 2026-08/09 畫面沒星號=**上游該兩月本來就沒給客服四人星號,不是 bug**;
     值機📱同理(該兩月 duties 屬其他組)。收到「星號/值機消失」回報時,先查上游該月有無四人資料再動手。
   - 想改後端行為(含鏡射欄位增減):**在 PR 描述寫需求**,由維護團隊改正式後端並部署;
     禁止改 dev-server 後宣稱正式站已修好。

14. **資料內容問題(截斷/亂碼/缺漏)一律屬資料層,禁止前端修補(2026-08-21)**:
   實例:「日誌 NEW」問題描述以 `...` 結尾,PR #239 假設列表端點回摘要、加了「單筆補查」——
   事後資料庫直接查證:**截斷值就存在正式資料庫裡,單筆讀回同一條截斷字串**,該機制抓不回任何全文。
   真因是 2026-08-12 歷史匯入誤存上游預覽值,已於 2026-08-21 在資料層修復(577 筆補回全文,9 筆 emoji 例外人工處理)。
   - 原則:**列表與單筆端點讀的是同一份儲存值**,顯示不完整≠端點回摘要。
   - 遇資料內容異常:在 PR 描述回報(附資料 ID 與現象),由維護團隊查資料層;
     **禁止在前端加補查/改寫/猜測還原資料的邏輯**——那會掩蓋資料層問題,讓真修復更難發現。
   - 改顯示邏輯前,先假設「資料就是長這樣」,顯示層只負責忠實呈現。
