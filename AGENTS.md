...
12. **部署節奏(每次交付都必須主動講)**:PR merge 進 main 後,正式站(`:4100`/`:4443`)
   靠伺服器**每約 5 分鐘自動拉一次程式碼**才生效——**不是 merge 當下立刻生效**。
   因此每個 PR 描述、以及每次告訴使用者「改好了」時,**必須附上這句提醒**:
   「merge 後請等約 5 分鐘,再用 Ctrl+F5 硬重整驗證;還看不到變化=部署還沒輪到,
   **不是改壞也不是沒改到,請不要再改一版**。」使用者回報「沒生效」時,先問
   merge 了多久+有沒有硬重整,確認超過 5 分鐘仍異常才開始查修,禁止直接疊新改動。
