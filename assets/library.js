(() => {
  ...
  const gameMasterListColumns = [
    ['gameId','GAME ID'],['gameVersion','GAME VERSION'],['nameEn','LIST OF GAMES'],['manufacturer','MANUFACTURER'],
    ['denomination','DENOMINATION'],['gameType','GAME TYPE'],['lines','NO. OF LINES'],['betMin','BET MIN (PHP)'],
    ['betMax','BET MAX (PHP)'],['maxPrize','MAX PRIZE (PHP)'],['maxPrizeMultiplier','MAX PRIZE MULTIPLIER'],
    ['jackpotGroup','PROGRESSIVE JACKPOT GROUP'],['jackpotMin','JACKPOT RANGE MIN'],['jackpotMax','JACKPOT RANGE MAX'],
    ['jackpotReserve','JACKPOT RTP RESERVE %'],['jackpotIncrement','JACKPOT RTP INCREMENT %'],
    ['totalJackpotRtp','TOTAL JACKPOT RTP %'],['baseRtp','BASE GAME RTP %'],['totalPayout','TOTAL PAYOUT % (THEORETICAL)']
  ];
  ...
  function startSyncCountdown(latest,badge){ if(!badge)return; clearInterval(syncCountdownTimer); const tick=()=>{badge.classList.toggle('is-syncing',latest?.status==='processing');badge.classList.toggle('is-error',latest?.status==='failed');if(latest?.status==='processing'){badge.textContent='Google Sheets 同步中…';return;}if(latest?.status==='failed'){const detail=String(latest.error||'').trim();badge.title=detail;badge.textContent=detail?`同步失敗：${detail}｜5 分鐘後自動重試 ${formatCountdown(nextSyncSeconds(latest))}`:`同步失敗｜5 分鐘後自動重試 ${formatCountdown(nextSyncSeconds(latest))}`;return;}badge.textContent=`每 5 分鐘自動更新｜下次更新 ${formatCountdown(nextSyncSeconds(latest))}`;};tick();syncCountdownTimer=setInterval(tick,1000);}
  ...
  function renderGames(){
    const body=$('#gameTableBody'),head=$('#gameTableHeadRow'); if(!body)return;
    if(head) head.innerHTML=gameMasterListColumns.map(([,label])=>`<th>${escapeHtml(label)}</th>`).join('')+'<th>Groups</th><th>影響 Clients</th><th>Internal Note</th><th>操作</th>';
    const query=$('#gameSearch')?.value.trim().toLowerCase()||''; const status=$('#gameStatusFilter')?.value||'';
    const games=state.games.filter((g)=>(!status||g.status===status)&&(!query||[g.gameId,g.nameEn,g.gameVersion,g.manufacturer,g.gameType].some((v)=>String(v||'').toLowerCase().includes(query)))).sort((a,b)=>String(a.gameId).localeCompare(String(b.gameId),undefined,{numeric:true}));
    body.innerHTML=games.map((game)=>{
      const groups=groupIdsForGame(game.id).map((id)=>groupById(id)?.name||id); const clients=clientsForGame(game.id);
      const sourceCells=gameMasterListColumns.map(([key])=>{const value=game[key];return `<td>${escapeHtml(value==null||value===''?'—':value)}</td>`;}).join('');
      return `<tr>${sourceCells}<td>${tagList(groups)}</td><td>${clients.length} ${clients.length?`<span title="${escapeHtml(clients.map(c=>c.name).join(', '))}">位</span>`:''}</td><td>${escapeHtml(game.internalNote||'—')}</td><td><div class="library-actions"><button class="secondary" data-action="game-groups" data-id="${game.id}">群組設定</button></div></td></tr>`;
    }).join('')||`<tr><td class="library-empty" colspan="${gameMasterListColumns.length+4}">目前沒有符合條件的遊戲。</td></tr>`;
  }
