...
const textFormatToolbarHtml = (kind, styleValue = {}) => {
  ...
  return `<div class="designer-option-format-toolbar" data-field-style-toolbar="${kind}"><span class="designer-format-label">字體大小</span><button type="button" data-field-font-step="-1" aria-label="縮小字體">−</button><input type="number" min="10" max="32" value="${style.fontSize || 14}" data-field-font-size aria-label="字體大小"><button type="button" data-field-font-step="1" aria-label="放大字體">＋</button><button type="button" data-field-style-toggle="bold" class="${style.bold ? 'is-active' : ''}">粗體</button><button type="button" data-field-style-toggle="italic" class="${style.italic ? 'is-active' : ''}">斜體</button><button type="button" data-field-style-toggle="strike" class="${style.strike ? 'is-active' : ''}">刪除線</button><label>文字顏色<input type="color" data-field-text-color value="${style.color || '#1f2937'}"></label><label>背景顏色<input type="color" data-field-background-color value="${style.backgroundColor || '#ffffff'}"></label><label>水平對齊<select data-field-text-align><option value="left" ${style.textAlign === 'left' ? 'selected' : ''}>靠左</option><option value="center" ${style.textAlign === 'center' ? 'selected' : ''}>置中</option><option value="right" ${style.textAlign === 'right' ? 'selected' : ''}>靠右</option></select></label><label>垂直對齊<select data-field-vertical-align><option value="top" ${style.verticalAlign === 'top' ? 'selected' : ''}>靠上</option><option value="middle" ${!style.verticalAlign || style.verticalAlign === 'middle' ? 'selected' : ''}>置中</option><option value="bottom" ${style.verticalAlign === 'bottom' ? 'selected' : ''}>靠下</option></select></label><button type="button" data-clear-field-style>清除格式</button></div>`;
};
