...
const renderLayoutDesigner = () => {
  ...
  panel.innerHTML = `<div class="layout-designer"><div class="layout-toolbar"><label>欄數: <select id="gridCols">${colsSelect}</select></label><label>列數: <select id="gridRows">${rowsSelect}</select></label><button class="btn-save-layout primary" type="button">儲存排版</button></div><div class="layout-designer-main layout-designer-content"><div class="layout-grid-area"><div class="layout-grid" style="grid-template-columns: repeat(${layout.columns}, minmax(92px, 1fr));">${cells.join('')}</div><div class="layout-unplaced"><span>未放置的欄位：</span>${unplaced}</div></div><aside class="layout-preview-area"><div class="layout-preview"><h3>即時預覽</h3><div class="ragic-form-grid ragic-view-grid" style="--form-columns:${layout.columns}">${fields.filter((field) => placed.has(field.key)).map((field) => { const item = layout.fields[field.key]; return `<div class="ragic-view-field" style="--form-row:${item.row};--form-col:${item.col};--form-colspan:${item.colSpan};--form-rowspan:${item.rowSpan};"><div class="ragic-view-label">${escapeHtml(field.label || field.key)}</div><div class="ragic-view-value">${escapeHtml(designerPreviewValue(field))}</div></div>`; }).join('') || '<div class="designer-preview-empty">拖入欄位後顯示預覽</div>'}</div></div></aside></div></div>`;};
